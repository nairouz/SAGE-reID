...
#         self.expert_input_gate = nn.Parameter(torch.zeros(129 * out_features))        
#         self.tau = nn.Parameter(torch.ones(1))
#         self.softmax = nn.Softmax(1)
#         #nn.init.kaiming_uniform_(self.expert_input_gate, a=math.sqrt(5))

#         if r > 0:
#             self.lora_A = nn.Parameter(self.weight.new_zeros((self.num_experts, r, in_features)))
#             self.lora_B = nn.Parameter(self.weight.new_zeros((self.num_experts, out_features, r)))
...
#         #idea 1
#         result1 = F.linear(x, T(self.weight), bias=self.bias)  
        
#         result = (self.lora_dropout(x).unsqueeze(1) @ self.lora_A.transpose(1, 2) @ self.lora_B.transpose(1, 2)) * self.scaling
#         Omega = result / (torch.norm(result, dim=-1, keepdim=True) + 1e-6)
#         Omega = Omega.view(Omega.size(0),Omega.size(1),-1)
#         epsilon = (Omega @ self.expert_input_gate) 
#         Gates = self.softmax(epsilon / self.tau)
        
#         self.gateing_values.append(Gates.mean(dim=0).detach().cpu().numpy())  # Accumulate
    
        
#         Gates = Gates.unsqueeze(-1).unsqueeze(-1)

#         result = result1 + torch.sum(result * Gates,dim=1)
#         #result = result1 + (torch.sum(result,dim=1) / 3)
#         return result
...
class Mlp(nn.Module):
    ...
    """
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.,lintype=None):
        super().__init__()
        ...
        if lintype == "lora":
            self.fc1 = lora.Linear(in_features, hidden_features,r=8, lora_alpha=32)
            self.fc2 = lora.Linear(hidden_features, out_features,r=8, lora_alpha=32)
        if lintype == "gate":
            self.fc1 = Linear(in_features, hidden_features,r=8, lora_alpha=32)
            self.fc2 = Linear(hidden_features, out_features,r=8, lora_alpha=32)
        self.act = act_layer()
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.,lintype=None):
        super().__init__()
        ...
        if lintype == "lora":
            self.qkv = lora.Linear(dim, dim * 3, bias=qkv_bias,r=8, lora_alpha=32)
            self.proj = lora.Linear(dim, dim ,r=8, lora_alpha=32)
        if lintype == "gate":
            self.qkv = Linear(dim, dim * 3, bias=qkv_bias,r=8, lora_alpha=32)
            self.proj = Linear(dim, dim ,r=8, lora_alpha=32)
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm,lintype=None):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop,lintype=lintype)
        # NOTE: drop path for stochastic depth, we shall see if this is better than dropout here
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop,lintype=lintype)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 act_layer=None, weight_init='',lintype=None):
        """
        Args:
        ...
        self.blocks = nn.ModuleList([Block(
                ...
                attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer,lintype=lintype)
        ...
        # Classifier head(s)
        self.head = nn.Linear(self.num_features, num_classes[0]) if num_classes[0] > 0 else nn.Identity()

        self.head_dist = None
        if distilled:
            self.head_dist = nn.Linear(self.embed_dim, self.num_classes) if num_classes > 0 else nn.Identity()

        # adds for person reid 
        # -- 
        self.classifier = nn.Linear(self.num_features, self.num_classes[0], bias=False)
        ...
        nn.init.constant_(self.feat_bn.bias, 0.0)
    ...
    def forward(self, x, training=False, register_hook=False):
