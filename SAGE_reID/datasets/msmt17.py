...
class Dataset_MSMT(object):
    ...
    def load(self, verbose=True):
        ...
        nametrain= osp.join(exdir, 'mask_train_v2')
        nametest = osp.join(exdir, 'mask_test_v2')
        ...
        if verbose:
            print(self.__class__.__name__, "v2~~~ dataset loaded")
            print("Dataset statistics:")
            print("  ----------------------------------------")
            print("  subset   | # ids | # images | # cameras")
            print("  ----------------------------------------")
            print("  train    | {:5d} | {:8d} | {:9d}".format(self.num_train_pids, len(self.train), 15))
            print("  query    | {:5d} | {:8d} | {:9d}".format(len(query_pids), len(self.query), 15))
            print("  gallery  | {:5d} | {:8d} | {:9d}".format(len(gallery_pids), len(self.gallery), 15))
            print("  ----------------------------------------")
